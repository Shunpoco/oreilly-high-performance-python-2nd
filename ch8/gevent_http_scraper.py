...
def download(url: str, semaphore: Semaphore):
    with semaphore:
        with closing(urllib.request.urlopen(url)) as data:
            return data.read()

def chunked_requests(urls: List[str], chunk_size=100):
    """
    Given an iterable of urls, this function will yield back the contents of the URLs.
    The requests will be batched up in "chunk_size" batches using a semaphore.
    """
...
if __name__ == "__main__":
    ...
    delay = 100
    num_iter = 1000
    base_url = f"http://127.0.0.1:8080?name=gevent&delay={delay}&"
